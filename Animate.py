...
class Animate:
	...
	def _animate(self, frame_num):
		...
		sys.stdout.write("\rEstimated temperature:\t%d degrees C " % estimate)
		sys.stdout.flush()
		return self._update_plot()

	def _setup_plot(self):
		self._fig, (self._ax1, self._ax2) = plt.subplots(nrows=2, ncols=1)
		plt.tight_layout(pad=3, w_pad=0, h_pad=3)
		self._fig.suptitle("Recursive Bayesian Estimator", fontsize=self._font_size)
		self._fig.canvas.set_window_title("Recursive Bayesian Estimator")
	...
	def _setup_axes(self):
		...
		self._ax1.set_ylabel("temperature (degrees C)", fontsize=self._font_size)
		self._ax1.set_title("temperature sensor reading", fontsize=self._font_size)
		self._line_actual, = self._ax1.plot([], [], "g")
		self._line_measured, = self._ax1.plot([], [], "r")
		self._line_result, = self._ax1.plot([], [], "b")
		self._ax1.legend(["actual", "measured", "result"], fontsize=self._font_size)
		# setup bottom plot - posterior probability distribuition histogram
		self._ax2.axis([0, self._M, 0, 1])
		self._ax2.set_xlabel("temperature (degrees C)", fontsize=self._font_size)
		self._ax2.set_ylabel("probability", fontsize=self._font_size)
		self._ax2.set_title("posterior probability distribution", fontsize=self._font_size)
