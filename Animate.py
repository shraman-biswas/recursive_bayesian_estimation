...
class Animate:
	...
	def _animate(self, frame_num):
		...
		sys.stdout.write(
			"\rEstimated temperature:\t%d degrees C " % estimate)
		sys.stdout.flush()
		return self._update_plot()

	# setup animation plots
	...
	def _setup_axes(self):
		...
		self._ax1.set_ylabel(
			"temperature (degrees C)", fontsize=self._font_size)
		self._ax1.set_title(
			"temperature sensor reading", fontsize=self._font_size)
